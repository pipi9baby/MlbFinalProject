...
path = "/home/mlb/users/q56074077/course/signalp/nonKNNData"
...
# reshape the data
X_Train = trainX.reshape(len(trainX)//(20 * 35), 35, 20, 1).astype('float32')
SPEuk_Test = SPEuk.reshape(len(SPEuk)//(20 * 35), 35, 20, 1).astype('float32')
TMEuk_Test = TMEuk.reshape(len(TMEuk)//(20 * 35), 35, 20, 1).astype('float32')
NCEuk_Test = NCEuk.reshape(len(NCEuk)//(20 * 35), 35, 20, 1).astype('float32')
y_Train = trainY
  
model = Sequential()
# Create Conv layer 1  
model.add(Conv2D(filters=1, kernel_size=(3, 3), padding='same', input_shape=(35, 20, 1), activation='relu'))  
...
# difine the method for training
