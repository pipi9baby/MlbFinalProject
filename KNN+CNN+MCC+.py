...
# reshape the data
X_Train = trainX.reshape(len(trainX)//(20 * 35), 35, 20).astype('float32')
SPEuk_Test = SPEuk.reshape(len(SPEuk)//(20 * 35), 35, 20).astype('float32')
TMEuk_Test = TMEuk.reshape(len(TMEuk)//(20 * 35), 35, 20).astype('float32')
NCEuk_Test = NCEuk.reshape(len(NCEuk)//(20 * 35), 35, 20).astype('float32')
y_Train = trainY
...
# Create Conv layer 1  
model.add(Conv1D(filters=200, kernel_size=8, padding='same', input_shape=(35, 20), activation='relu', bias_initializer=init))  
...
# Create Conv layer 2  
model.add(Conv1D(filters=180, kernel_size=8, padding='same', activation='relu', bias_initializer=init))  
#model.add(Conv2D(filters=20, kernel_size=(5, 5), padding='same', activation='relu', bias_initializer=init))  
# Create Max-Pool 2  
model.add(AveragePooling1D(pool_size=2, strides=2))

# Create Conv layer 3 
model.add(Conv1D(filters=160, kernel_size=8, padding='same', activation='relu', bias_initializer=init))
...
# train
model.fit(x=X_Train, y=y_Train, validation_split=0.2, epochs=10, batch_size=90, verbose=2)
