...
# reshape the data
X_Train = trainX.reshape(len(trainX)//(20 * 96), 96, 20).astype('float32')
SPEuk_Test = SPEuk.reshape(len(SPEuk)//(20 * 96), 96, 20).astype('float32')
TMEuk_Test = TMEuk.reshape(len(TMEuk)//(20 * 96), 96, 20).astype('float32')
NCEuk_Test = NCEuk.reshape(len(NCEuk)//(20 * 96), 96, 20).astype('float32')
y_Train = trainY
  
model = Sequential()

# Create Conv layer 1  
model.add(Conv1D(filters=100, kernel_size=8, padding='same', input_shape=(96, 20), activation='relu', bias_initializer=init))  
...
# Create Conv layer 2  
model.add(Conv1D(filters=80, kernel_size=8, padding='same', activation='relu', bias_initializer=init))  
